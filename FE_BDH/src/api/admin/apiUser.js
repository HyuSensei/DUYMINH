...
const getUserByUserName = async (req, res) => {
  try {
    ...
    let dataUser = await axios.get(
      process.env.BASE_URL + `userByUserName/${userName}`
    );
...
const paginationUser = async (req, res) => {
  try {
    ...
    let countUser = await axios.get(process.env.BASE_URL + `countUser`);
    let dataUser = await axios.get(
      process.env.BASE_URL + `user/limit/${currentPage}`
    );
    let data = dataUser.data.users;
    return res.render("admin/userAdmin.ejs", {
      dataUser: data,
...
const indexUser = async (req, res) => {
  try {
    ...
    let data_product = await axios.get(process.env.BASE_URL + `admin/users`, {
      params,
    });
    return res.render("admin/user.ejs", {
      users: data_product.data.users,
